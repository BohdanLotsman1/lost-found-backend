...
export class TokenService {
    ...
    async isTokenBlacklisted( token: string, user_id: string ): Promise<boolean> {
        return (await this.connection.table(this.blacklistedTokensTable)
            .where('user_id', user_id)
    ...
    async destroy(access_token: string,  ) {
        await this.connection.table( this.accessTokensTable )
            .delete()
            .where('access_token', access_token )
